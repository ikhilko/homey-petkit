...
class PetKitClient {
  ...
  async refreshSession() {
    ...
    try {
      const response = await this.client.post(
        `${this.baseUrl}${PETKIT_ENDPOINTS.REFRESH_SESSION}`,
  ...
  async _getPetDetails() {
    ...
    try {
      const response = await this.client.get(
        `${this.baseUrl}${PETKIT_ENDPOINTS.DETAILS}`,
        { headers }
      );
  ...
  async _getAccountData() {
    ...
    try {
      const response = await this.client.get(
        `${this.baseUrl}${PETKIT_ENDPOINTS.FAMILY_LIST}`,
        { headers }
      );
  ...
  async getDevicesData() {
    ...
    const { mainTasks, recordTasks, mediaTasks, liveTasks } = this._prepareTasks(deviceList);
  ...
  async _fetchDeviceData(device, dataClass) {
    ...
    // Get endpoint based on data class and device type
    let endpoint = this._getEndpointForDataClass(dataClass, deviceType);
    if (!endpoint) {
      console.log(`Endpoint not found for device type: ${deviceType}, data class: ${dataClass}`);
      return;
    }

    // Get query parameters
    const queryParam = this._getQueryParam(device, dataClass);

    try {
      const response = await this.client.post(
        `${this.baseUrl}${deviceType}/${endpoint}`,
      ...
      if (!responseData) {
        console.log(`No data returned for ${deviceType}/${endpoint}`);
        return;
      }
      ...
      this._handleDataResponse(device, responseData, dataClass, deviceType);

    } catch (error) {
      console.error(`Failed to fetch device data for ${deviceType}/${endpoint}: ${error.message}`);
    }
  }
  ...
  _getEndpointForDataClass(dataClass, deviceType) {
    ...
    const endpointMap = {
      'Feeder': PETKIT_ENDPOINTS.DEVICE_DATA,
      'Litter': PETKIT_ENDPOINTS.DEVICE_DATA,
      'WaterFountain': PETKIT_ENDPOINTS.DEVICE_DATA,
      'Purifier': PETKIT_ENDPOINTS.DEVICE_DATA,
  ...
  _getQueryParam(device, dataClass) {
    // Basic query parameters - can be enhanced based on specific needs
    const params = {
      id: device.deviceId
    };

    // Add specific parameters based on data class
  ...
  async sendApiRequest(deviceId, action, setting = null) {
    ...
    try {
      ...
      // Map actions to endpoints and parameters
      switch (action) {
        case 'manual_feed':
        ...
        case 'start_clean':
          ...
          params = { id: deviceId, cmd: 0 }; // 0 = cleaning
          break;
        case 'control_device':
          endpoint = 'controlDevice';
          ...
          break;
        default:
          throw new PetkitAPIError(`Action ${action} not supported`);
      }
